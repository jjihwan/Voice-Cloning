...
def main():
    ...
    if args.input_dir is None:
        ...
        args.input = sorted(glob(args.input_dir + '/*'))

    for input in args.input:
        print(f'loading wave source from {input}...')
        X, sr = librosa.load(
            input, sr=args.sr, mono=False, dtype=np.float32, res_type='kaiser_fast'
        )
        ...
        vocal_dir = os.path.join(args.output_dir, 'vocals/')
        instrument_dir = os.path.join(args.output_dir, 'instruments/')
        ...
        print('done')
        sf.write('{}{}_Instruments.wav'.format(instrument_dir, basename), wave.T, sr)

        print('inverse stft of vocals...', end=' ')
        wave = spec_utils.spectrogram_to_wave(v_spec, hop_length=args.hop_length)
        print('done')
        sf.write('{}{}_Vocals.wav'.format(vocal_dir, basename), wave.T, sr)

        if args.output_image:
            image = spec_utils.spectrogram_to_image(y_spec)
            utils.imwrite('{}{}_Instruments.jpg'.format(vocal_dir, basename), image)

            image = spec_utils.spectrogram_to_image(v_spec)
            utils.imwrite('{}{}_Vocals.jpg'.format(instrument_dir, basename), image)
